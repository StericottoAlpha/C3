...
OpenAI GPT を使用した会話型AI（LangChain ReActエージェント）
"""
import logging
import hashlib
import json
...
class ChatAgent:
    ...
    OpenAI GPT-4o-mini を使用した会話型AIエージェント
    """
    ...
    def __init__(
        ...
        base_url: str = "http://localhost:11434"  # Ollama用（オプション）
    ):
        """
        Args:
            model_name: OpenAIモデル名（デフォルト: "gpt-4o-mini"）
            temperature: 温度パラメータ（0.0-1.0、推奨: 0.0-0.2）
            use_openai: OpenAI APIを使用（デフォルト: True）
            openai_api_key: OpenAI APIキー
            base_url: Ollama APIのURL（use_openai=Falseの場合のみ使用）
        """
    ...
    def _initialize_llm_streaming(self):
        ...
        if self.use_openai:
            ...
            logger.info(f"Initialized streaming ChatOpenAI")
            return llm
        else:
            # Ollamaはストリーミング非対応（現状）
            return self.llm

    def _initialize_llm(self):
        """LLMを初期化（OpenAI GPTを使用）"""
        if self.use_openai:
            # OpenAI GPT使用（推奨）- ネイティブツールコーリング対応
            llm = ChatOpenAI(
                model=self.model_name,
                temperature=self.temperature,
                api_key=self.openai_api_key
            )
            logger.info(f"Initialized ChatOpenAI with model={self.model_name}, temp={self.temperature}")
            return llm
        else:
            # Ollama使用（オプション・実験的）
            if not OLLAMA_AVAILABLE:
                raise ImportError(
                    "⚠️ Ollama関連のパッケージがインストールされていません。\n"
                    "USE_OPENAI=False の場合、以下のパッケージが必要です:\n"
                    "pip install langchain-community langchain-experimental\n\n"
                    "本番環境ではUSE_OPENAI=True を推奨します。"
                )
            logger.warning("⚠️ Ollamaモードで実行中（実験的機能）")
